...
class MeasurementApp(QMainWindow):
    def __init__(self):
        ...
        # Load Mask R-CNN model (ensure the model files are in the same directory or update the paths)
        # You can download the model files from:
        # https://github.com/opencv/opencv_extra/tree/master/testdata/dnn
        self.mask_rcnn_model = cv2.dnn.readNetFromTensorflow("frozen_inference_graph.pb", "mask_rcnn_inception_v2_coco_2018_01_28.pbtxt")
    ...
    def initUI(self):
        self.setWindowTitle("Object Measurement System with Mask R-CNN")
            ...
            if self.roi:
                ...
                if self.mode == "measuring":
                    if y1 < y2 and x1 < x2 and y1 >= 0 and x1 >= 0 and y2 <= frame.shape[0] and x2 <= frame.shape[1]:
                        ...
                        # Use Mask R-CNN for detection and segmentation in the ROI
                        roi_h, roi_w = roi_frame.shape[:2]
                        blob = cv2.dnn.blobFromImage(roi_frame, swapRB=True, crop=False)
                        self.mask_rcnn_model.setInput(blob)
                        # Forward pass: obtain detections and corresponding masks
                        detections, masks = self.mask_rcnn_model.forward(["detection_out_final", "detection_masks"])
                        ...
                        for i in range(detections.shape[2]):
                            ...
                            if score > 0.5:
                                ...
                                # Adjust bounding box coordinates relative to the main frame
                                startX += x1
                                startY += y1
                                endX += x1
                                endY += y1
                                
                                cv2.rectangle(display_frame, (startX, startY), (endX, endY), contour_color, 2)
                                cx = (startX + endX) // 2
                                cy = (startY + endY) // 2
                                text = f"{length_mm:.1f} {unit}"
                                (text_width, text_height), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
                                cv2.rectangle(display_frame, (cx - 5, cy - text_height - 5), (cx + text_width + 5, cy + 5), (0, 0, 0), -1)
                                cv2.putText(display_frame, text, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                        
                        # Draw detection statistics on the main frame
                        cv2.putText(display_frame, f"Total objects: {total_objects}", (10, 30),
